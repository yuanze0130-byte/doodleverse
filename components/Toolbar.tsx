...
const ToolButton: React.FC<{
    ...
    <button
        ...
        onClick={onClick}
        disabled={disabled}
        className={`${baseButtonClass} ${active ? activeButtonClass : ''} disabled:cursor-not-allowed disabled:opacity-40`}
    >
        {icon}
    </button>
);
...
const ToolGroupButton: React.FC<{
...
}> = ({ label, activeTool, setActiveTool, items, fallbackIcon }) => {
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            ...
            />
            {open && (
                <div className="absolute left-full top-0 ml-3 flex flex-col gap-2 rounded-[24px] border border-neutral-200 bg-white p-2 shadow-[0_24px_50px_rgba(15,23,42,0.16)]">
                    {items.map(item => (
                        <ToolButton
                            key={item.id}
                            label={item.label}
...
export const Toolbar: React.FC<ToolbarProps> = ({
    ...
    return (
        <div
            className="absolute top-4 z-[40] flex flex-col items-center gap-2 rounded-[28px] border border-neutral-200 bg-[#171717] px-2 py-3 text-white shadow-[0_24px_60px_rgba(15,23,42,0.28)]"
            style={{
        ...
        >
            ...
            <input
                ...
                className="h-10 w-10 cursor-pointer rounded-2xl border border-white/10 bg-transparent p-0"
            />
            <input
                type="range"
                min="1"
